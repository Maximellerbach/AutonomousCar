...
def generate_functions(X, Y, proportion=0.25):
    functions = (change_brightness, rescut, inverse_color, night_effect, add_random_shadow, add_rdm_noise)
    length_X = len(X)

    X_aug = []
    Y_aug = []
    for f in functions:
        indexes = np.random.choice([True, False], length_X, p=[proportion, 1 - proportion])
...
def generate_functions_replace(
    X, Y, proportion=0.25, functions=(change_brightness, rescut, inverse_color, night_effect, add_random_shadow, add_rdm_noise)
):
    X = np.array(X)
    Y = np.array(Y)
...
def generate_horizontal_flip(Dataset, names2index, flipable_components, X, Y, proportion=0.25):
    ...
    for index in range(len(X)):
        if indexes[index]:
            im, Y_aug = horizontal_flip(Dataset, names2index, flipable_components, X[index], Y_aug, index)
            X_aug.append(im)
