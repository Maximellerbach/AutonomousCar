...
def visualize_fe_output(self, img, show=True, sleep_time=1):
...
def visualize_model_layer_filter(
    model, img, layer_index, output_size=None, mult=1, layer_outputs=None, tmp_model=None, show=True, waitkey=None
):
...
def get_saliency(model, inputs, class_idx, image_input_index=0, **kwargs):
    ...
    saliency_map = saliency(loss, X, **kwargs)
...
def get_gradcam(model, inputs, class_idx, penultimate_layer=-1, **kwargs):  # need to work on activation modifier
    ...
    cam = gradcam(loss, X, penultimate_layer=penultimate_layer, **kwargs)
...
def vis_layer(model, layer_name, filter_number, **kwargs):
    def model_modifier(current_model):
        ...
        new_model = tensorflow.keras.Model(inputs=current_model.inputs, outputs=target_layer.output)
    ...
    activation = activation_maximization(loss, callbacks=[Print(interval=50)], **kwargs)[0]
