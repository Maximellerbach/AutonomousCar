...
class auto_labeling(sim_client.SimpleClient):
    def __init__(self, dataset, host="127.0.0.1", port=9091):
        ...
        self.pos_offset_values = [(i / self.n_lateral) * self.road_width for i in range(-self.n_lateral, self.n_lateral + 1)]
        print(self.pos_offset_values)

        self.rot_offset_values = [(i / self.n_rot_y) * self.max_rot for i in range(-self.n_rot_y, self.n_rot_y + 1)]
    ...
    def loop(self):
        ...
        while True:
            ...
            node_rotation = np.quaternion(
                self.node_info.get("Qx"), self.node_info.get("Qy"), self.node_info.get("Qz"), self.node_info.get("Qw")
            )
            node_position = np.array(
                [self.node_info["pos_x"], self.node_info["pos_y"] + self.height_offset, self.node_info["pos_z"]]
            )

            # point ahead
            self.await_get_active_node_coords(index + 2)
            target_node_rotation = np.quaternion(
                self.node_info.get("Qx"), self.node_info.get("Qy"), self.node_info.get("Qz"), self.node_info.get("Qw")
            )
            target_node_position = np.array(
                [self.node_info["pos_x"], self.node_info["pos_y"] + self.height_offset, self.node_info["pos_z"]]
            )

            for pos_x_offset in self.pos_offset_values:
                offset_pos = np.array([pos_x_offset + self.width_offset, 0, 0])
                offset_pos = quaternion.rotate_vectors(node_rotation, offset_pos)
                ...
                for rot_y_offset in self.rot_offset_values:
                    ...
                    self.move_car(car_pos[0], car_pos[1], car_pos[2], qx=car_rot.x, qy=car_rot.y, qz=car_rot.z, qw=car_rot.w)
                    ...
                    self.save_img(self.image, direction=steering, time=time.time())
