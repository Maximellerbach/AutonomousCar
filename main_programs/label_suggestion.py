...
class LabelisationSuggestion:
    ...
    def compare_lat(self, lat_to_compare, lat_imgs, N=5):
        distances = [self.compute_distance(lat_to_compare, lat_img) for lat_img in lat_imgs]
    ...
    def main(self, paths, lat_imgs, i):
        path = paths[i]
        distances, smallest_distance = self.compare_lat(lat_imgs[i], lat_imgs)
        indexes = []
        ...
        predicted = self.estimate_annotation(annotations, smallest_distance)
...
if __name__ == "__main__":
    ...
    lab_helper = LabelisationSuggestion(Dataset, "test_model\\models\\forza4.h5")

    lab_helper.iterate_main(f"{base_path}\\ironcar\\0_1618348458.6400757\\")
