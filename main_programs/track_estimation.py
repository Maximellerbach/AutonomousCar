...
class TrackEstimation:
    ...
    def get_pos(self, Y, time_series, speed_series):
        ...
        for it, st in enumerate(Y):
            ...
            degree_angle = math.pi * st * self.steer_coef * (speed_series[it] * time_series[it])
    ...
    def draw_points(self, pos_list, degs, colors=[(1, 0, 0), (0, 1, 0), (0, 0, 1)], thickness=1):
        ...
        for pos, deg in zip(pos_list, degs):
            ...
            cv2.circle(self.pmap, (int(rpx), int(rpy)), 1, color, thickness=thickness)
...
if __name__ == "__main__":
    ...
    iner_bound_list, outer_bound_list = Estimator.boundaries(pos_list, radius=5)
    ...
    Estimator.draw_segments(turns_segments, matches=matchs, min_max=iner_bound_list + outer_bound_list)

    Estimator.draw_circuit(pos_list, directions, colors=np.array(((0.75, 0, 0), (1, 1, 1), (0, 0, 0.75))))
