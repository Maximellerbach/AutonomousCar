...
class Labeliser:
    def __init__(self, Dataset, output_components, dos, mode="union"):
        ...
        self.output_components_names = self.Dataset.indexes2components_names(self.output_components)
        ...
        self.annotation_img_paths = [self.Dataset.get_img_path(annotation_path) for annotation_path in self.annotation_paths]
        ...
        layout = [
            [sg.Text(f"working dir: {self.dos}")],
            [sg.Graph((160, 120), (0, 120), (160, 0), enable_events=True, key="__GRAPH__")],
        ]

        layout += [[sg.Text(name), sg.InputText(key=name)] for name in self.output_components_names]
        ...
        layout += [
            [
                ...
                sg.Text("image {i}/{tot}" + " " * 8, key="__INDEX__"),
            ]
        ]  # add some blank spaces to avoid overflow
    ...
    def main(self):
        if self.mode == "difference":
            to_label_paths = set(self.annotation_img_paths) - set(self.img_paths)
        ...
        while i < to_label_len - 1:
            ...
            self.window["__GRAPH__"].draw_image(filename=img_path, location=(0, 0))

            self.window["__INDEX__"].update(f"image {i+1}/{to_label_len}")

            try:
                annotation_path = self.img_paths_mapping[img_path]
                annotation = self.Dataset.load_annotation(annotation_path, to_list=False)
            except KeyError:
            ...
            for component_name in self.output_components_names:
                self.window[component_name].update(str(annotation[component_name]))
            ...
            while True:
                ...
                if event == sg.WIN_CLOSED:
                    ...
                    list_of_point = component.from_string(values[component.name])
                    ...
                    if component.name == "left_lane":
                        ...
                        right_list_of_point = component.from_string(values["right_lane"])
                        self.window["__GRAPH__"].draw_lines(list_of_point, color="blue")
                        self.window["__GRAPH__"].draw_lines(right_list_of_point, color="red")
                    elif component.name == "right_lane":
                        self.window["__GRAPH__"].draw_image(filename=img_path, location=(0, 0))

                        left_list_of_point = component.from_string(values["left_lane"])
                        self.window["__GRAPH__"].draw_lines(list_of_point, color="red")
                        self.window["__GRAPH__"].draw_lines(left_list_of_point, color="blue")

                    pt_list_iteration = (pt_list_iteration + 1) % (2 * len(self.iterable_components))

                elif event == "__SAVE__":
                    to_save = annotation
                    for cmp_key in values:
                        if "__" not in cmp_key:
                            to_save[cmp_key] = self.Dataset.get_component(cmp_key).from_string(values[cmp_key])

                    to_save["dos"] = self.dos
                    to_save["img_path"] = img_path
                    new_annotation_path = self.Dataset.save_annotation_dict(to_save)
...
if __name__ == "__main__":
    ...
    Dataset = dataset_json.Dataset(["direction", "speed", "throttle", "left_lane", "right_lane"])
